...
import PriceBreakdown, { formatINR } from "./PriceBreakdown";
...
export default function BookingDialog({ open, worker, serviceSlug, onClose, onBooked }) {
  ...
  const loadQuote = useCallback(async () => {
    if (!worker?.id || !serviceSlug) return;

    setQuoteLoading(true);
    try {
      const result = await bookingsApi.quote({ staffId: worker.id, serviceSlug, quantity });
  ...
  }, [worker?.id, serviceSlug, quantity]);
