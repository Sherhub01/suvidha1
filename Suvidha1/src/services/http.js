...
/** Clears the signed-in session from both storages and returns to a login page. */
...
function createClient({ path = "", admin = false } = {}) {
  ...
  client.interceptors.request.use((config) => {
    const token = admin
      ? adminSession.getToken() || localStorage.getItem("admin_token")
      : session.getToken() || localStorage.getItem("token");
