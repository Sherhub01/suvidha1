...
import { LayoutGrid, Wrench, Info, Settings, Zap, LogOut, CalendarCheck, Map, Bell, ChevronDown, Phone } from "lucide-react";
...
export default function Sidebar({ collapsed, onToggle }) {
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-50 flex flex-col bg-gradient-to-b from-slate-900 via-indigo-950 to-slate-900 border-r border-white/6 shadow-2xl transition-all duration-300 ease-in-out ${collapsed ? "-translate-x-full lg:translate-x-0 lg:w-20" : "translate-x-0 w-64"}`}>
      <button onClick={onToggle}
        ...
        {!collapsed && (
          <div className="text-left">
            <span className="block text-lg font-bold text-white tracking-tight leading-none">Suvidha<span className="text-amber-400">1</span></span>
      ...
      <nav className="flex-1 space-y-1 px-2 py-4 overflow-y-auto scrollbar-none">
        {NAV_ITEMS.map(({ to, label, icon: Icon }) => (
          <NavLink key={to} to={to} title={collapsed ? label : undefined}
            className={({ isActive }) =>
              `relative flex items-center gap-3 rounded-xl transition-all duration-200 group ${collapsed ? "justify-center px-0 py-3" : "px-3 py-2.5"} ${isActive ? "bg-amber-400/15 text-amber-400" : "text-white/55 hover:bg-white/6 hover:text-white"}`
            }>
            {({ isActive }) => (
              <>
                <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg transition-colors ${isActive ? "bg-amber-400/20" : "bg-white/5 group-hover:bg-white/10"}`}>
      ...
      <div className="border-t border-white/6 px-2 py-4 space-y-2">
        <div ref={dropRef} className="relative">
          <button onClick={() => setDropOpen(v => !v)} title={collapsed ? fullName : undefined}
            className={`flex w-full items-center gap-3 rounded-xl bg-white/5 hover:bg-white/10 transition cursor-pointer ${collapsed ? "justify-center px-0 py-2.5" : "px-3 py-2.5"}`}>
          ...
          {dropOpen && (
            <div className={`absolute z-50 w-52 rounded-2xl border border-white/10 shadow-2xl py-2 ${collapsed ? "left-full ml-2 bottom-0" : "bottom-full mb-2 left-0 right-0"}`}
